...
def compute_total_loss(model, cost_pred, mean, logvar, z, labels, feature, config, return_components=True):
    ...
    infonce = infonce_loss(z, labels, tau=config.get('tau', 0.1), tau_c=config.get('tau_c', None))
    
    # Weighted sum
    total = config['lambda_reg'] * reg + config['lambda_pair'] * pair + config['gamma'] * smooth + config['beta'] * kld + config.get('lambda_infonce', 0.01) * infonce
...
def make_vae_reg_model(vae, config, input_dim, latent_dim, hidden_dim, y_std, verbose=True):
    ...
    return vae_cost_model, optimizer


# In[252]:


def lambda_pair_warmup(epoch: int, rank_warmup_epochs: int, lambda_pair_max: float) -> float:
    if rank_warmup_epochs <= 0:
        return lambda_pair_max
    t = min(max(epoch, 0), rank_warmup_epochs) / rank_warmup_epochs  # 0~1
...
def train_regression(vae_cost_model, optimizer, train_loader, val_loader, input_data_scaled, costs, config, top_k=10, use_rank=True, rank_warmup_epochs=200):
    ...
    for epoch in range(1, config['epochs']+1):
        ...
        for x_batch, labels in train_loader:
            ...
            config['lambda_pair'] = lambda_pair_warmup(epoch, rank_warmup_epochs, lambda_pair_max)
...
import pandas as pd
import datetime
sys.path.append("/root/work/tenset/scripts/unfolding/util_modules")
import extent.csv as utils_csv
...
filename = f"{result_dir}/vae_extent_{now}.csv"
csv_logger = utils_csv.CSVLogger(filename, top_k=top_k)
...
sampling_hyper = {
    ...
    "uncertainty_topk": [64, 128],    # 몇 개부터 불확실성 기반 선택을 할지
    "grad_num": [4],
    "rand_num": [0],
    ...
    "rank_warmup_epochs" : [100],

    "lambda_reg" : [0.01],
    "lambda_pair" : [2.0, 3.0],
    "lambda_infonce" : [0.1, 0.2],
    "gamma" : [0.01],
    "beta" : [0.01],
...
to_measure_configs = csv_logger.filter_already_measured(f"{result_dir}/vae_extent_total_avg.csv", sampling_hyper)
...
for hyper_config in to_measure_configs:
    ...
    w_cost, w_unc, w_div = hyper_config["weights"]
    print(f"weights: {hyper_config['weights']}")
    ...
    hyperparameter = hyper_config
    hyperparameter['feature_predictor_lr'] = 0.0
    ...
    for phase in range(1, len(input_data_scaled) // measure_size + 1):
        ...
        vae_cost_model, optimizer = make_vae_reg_model(vae, hyperparameter, input_dim, latent_dim, hidden_dim, y_std, verbose=False)
        ...
        vae_cost_model, topk_recall_signal, val_reg_r2, val_rank_r2 = train_regression(vae_cost_model, optimizer, train_loader, val_loader, 
                                                                                       input_data_scaled, costs, hyperparameter, top_k=top_k, use_rank=False, rank_warmup_epochs=hyper_config["rank_warmup_epochs"])
        ...
        if break_signal:
            print("학습한 데이터 수 :", phase * measure_size)
            used_time = time.time() - tic
            print(f"총 측정 시간: {used_time:.2f} 초")
            print("=============================================")

            df_results = csv_logger.append_result(hyper_config, phase, used_time, topk_recall_signal, reg_history, rank_history)
            
            csv_logger.save_result()

            csv_logger.save_result_avg()
