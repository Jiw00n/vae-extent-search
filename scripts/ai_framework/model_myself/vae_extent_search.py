...
csv_logger = CSVLogger(filename, top_k=top_k)
...
sampling_hyper = {
    ...
    "uncertainty_topk": [64],    # 몇 개부터 불확실성 기반 선택을 할지
    "grad_num": [4],
    "rand_num": [0],
...
to_measure_configs = csv_logger.filter_already_measured(f"{result_dir}/vae_extent_total.csv", sampling_hyper)
...
for hyper_config in to_measure_configs:
    ...
    hyperparameter = {
        ...
        'lambda_reg' : 0.01,
        'lambda_pair': hyper_config["lambda_pair"],
        'margin_scale': hyper_config["margin_scale"],
        'gamma': 0.01,
        'beta': 0.01,
        'noise_std': 0.001,
    ...
    for phase in range(1, len(input_data_scaled) // measure_size + 1):
        ...
        vae_cost_model, topk_recall_signal, val_reg_r2, val_rank_r2 = train_regression(vae_cost_model, optimizer, train_loader, val_loader, 
                                                                                       input_data_scaled, costs, config, top_k=top_k, use_rank=False, rank_warmup_epochs=hyper_config["rank_warmup_epochs"])
